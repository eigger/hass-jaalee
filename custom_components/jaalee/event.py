"""Support for jaalee event entities."""
...
class JaaleeEventEntity(EventEntity):
    """Representation of a Jaalee event entity."""
    ...
    def __init__(
        ...
        address: str,
        event_class: str,
        event: JaaleeBleEvent | None,
    ) -> None:
        """Initialise a Jaalee event entity."""
    ...
    @callback
    def _async_handle_event(self, event: JaaleeBleEvent) -> None:
        self._trigger_event(event[EVENT_TYPE], event[EVENT_PROPERTIES])
...
async def async_setup_entry(
    ...
    async_add_entities(
        # Matches logic in PassiveBluetoothProcessorEntity
        JaaleeEventEntity(address_event_class[0], address_event_class[2], None)
    ...
    @callback
    def _async_discovered_event_class(event_class: str, event: JaaleeBleEvent) -> None:
        """Handle a newly discovered event class with or without a postfix."""
        async_add_entities([JaaleeEventEntity(address, event_class, event)])
