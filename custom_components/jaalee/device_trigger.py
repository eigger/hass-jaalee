"""Provides device triggers for Jaalee BLE."""
...
def get_event_classes_by_device_id(hass: HomeAssistant, device_id: str) -> list[str]:
    ...
    Events for Jaalee BLE devices are dynamically discovered
    ...
    jaalee_config_entry = next(
        entry for entry in config_entries if entry and entry.domain == DOMAIN
    )
    return jaalee_config_entry.data.get(CONF_DISCOVERED_EVENT_CLASSES, [])
...
async def async_validate_trigger_config(
    ...
    if event_class not in event_classes:
        raise InvalidDeviceAutomationConfig(
            f"Jaalee trigger {event_class} is not valid for device_id '{device_id}'"
        )

    if event_type not in get_event_types_by_event_class(event_class):
        raise InvalidDeviceAutomationConfig(
            f"Jaalee trigger {event_type} is not valid for device_id '{device_id}'"
        )
...
async def async_get_triggers(
    ...
    """Return a list of triggers for Jaalee BLE devices."""
    event_classes = get_event_classes_by_device_id(hass, device_id)
    return [
        {
        ...
        event_trigger.TRIGGER_SCHEMA(
            {
                ...
                event_trigger.CONF_EVENT_TYPE: JAALEE_BLE_EVENT,
